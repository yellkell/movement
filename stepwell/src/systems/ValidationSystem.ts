...
// Johansen is explicit that varying gameplay by play-area size is "not viable
// (or even desirable)"; the well is authored against a fixed 2 × 2 m minimum.
// What neither exemplar ships is the courtesy of *saying so*: read the real
// room once, and if it can't hold the grid, warn plainly before the set —
// not when someone punches a wall mid-ride.
    ...
    session
      ...
      .then((space) => {
        ...
        if (w < PLAY_AREA.requiredWidth || d < PLAY_AREA.requiredDepth) {
          this.world
            .getSystem(EnvironmentSystem)
            ?.showPlayAreaWarning(w, d);
        }
      })
      .catch(() => {
