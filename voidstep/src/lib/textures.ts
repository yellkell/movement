...
// onto a canvas. Deterministic — no RNG in any of these. Cloned from
// stepwell, minus the call-plate glyph (no plates in the circuit).
...
/** Soft radial glow — the invitation circle. */
...
// The frame wears the void's magenta instead of the well's cyan.
export function textPanel(spec: PanelSpec): Mesh {
  ...
  g.fillRect(0, 0, W, h);
  g.strokeStyle = 'rgba(255,90,209,0.5)';
  g.lineWidth = 6;
  g.strokeRect(3, 3, W - 6, h - 6);

  const color = spec.color ?? '#e8a2ff';
  let y = pad;
  g.textAlign = 'center';
  if (spec.title) {
    g.fillStyle = '#ff5ad1';
  ...
  if (spec.small) {
    g.fillStyle = 'rgba(232,162,255,0.6)';
